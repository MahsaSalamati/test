...
public class FactorRepository : IFactorRepository
{
            ...
            try
            {
                ...
                using (SqlCommand command = new("Proc_Mnd_Kala", connection))
                {
                    ...
                    foreach (var item in factorItems)
                    {
                        ...
                        string whereString = "'" + returnedInvoice.FiscalYearBeginDate + "' AND '" + returnedInvoice.Dt_FBB + "'";
                        command.Parameters.AddWithValue("@where", whereString);
    ...
    public async Task<List<SaleServiceFactor>> GetServiceSaleFactorPrint(long? ForoshID, long? fiscalYear, int? UserId, int? Offset, int? ID_tbl_Bzy, string? Date1, string? Date2, object parameters = null!, CancellationToken cancellationToken = default)
    {
